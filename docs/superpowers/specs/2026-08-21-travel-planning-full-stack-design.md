...
密钥缺失或服务失败时，Agent 返回 `degraded` 的最小 `WeatherPlanData`，其 `daily` 可以为空元组；不得构造虚假逐日预报，并提示用户出行前复核官方天气与预警。`success` 天气结果必须至少包含 1 项逐日天气。
...
- **合同一致性：** 所有运行时数据遵循已合并的冻结旅行合同；天气 `degraded` 数据允许空逐日列表，天气 `success` 数据必须至少包含 1 项逐日天气；来源、警告与降级项由最终文档合同校验。
