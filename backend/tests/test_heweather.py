...
async def test_maps_forecast_and_uses_fixed_endpoint_and_params():
    route = respx.get(f"{BASE}/v7/weather/15d").mock(
...
async def test_non_200_weather_code_is_controlled_error():
    respx.get(f"{BASE}/v7/weather/15d").mock(return_value=httpx.Response(200, json={"code": "401"}))
...
async def test_transport_failures_are_retried_then_controlled(failure):
    route = respx.get(f"{BASE}/v7/weather/15d").mock(side_effect=failure)
...
async def test_429_exhaustion_records_failure_without_upstream_body():
    route = respx.get(f"{BASE}/v7/weather/15d").mock(return_value=httpx.Response(429, text="secret body"))
    weather_client = client(max_attempts=2)
...
async def test_invalid_weather_structure_is_controlled(payload):
    respx.get(f"{BASE}/v7/weather/15d").mock(return_value=httpx.Response(200, json=payload))
...
async def test_weather_days_fifteen_and_sixteen_share_effective_cache_key():
    route = respx.get(f"{BASE}/v7/weather/15d").mock(return_value=httpx.Response(200, json={
        "code": "200", "updateTime": "2026-08-20T10:00:00Z", "daily": [
...
async def test_weather_non_2xx_success_shaped_response_is_not_cached(status_code):
    cache = MemoryCache()
    route = respx.get(f"{BASE}/v7/weather/15d").mock(return_value=httpx.Response(status_code, json={
        "code": "200",
        "updateTime": "2026-08-20T10:00:00Z",
        "daily": [{"fxDate": "2026-09-01", "textDay": "晴", "tempMin": "20", "tempMax": "28"}],
    }))
