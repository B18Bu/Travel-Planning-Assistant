...
- `/api/health`、`/api/ready` 和 `/api/travel-plans`。

### 非目标与明确不接入

本轮不接入知识库、OTA（在线旅行代理平台）、酒店或餐厅交易、预订、支付、库存、实时价格、优惠、排队、评分和订单链接。模型使用 `extra="forbid"` 拒绝未声明字段，禁止把上述信息通过自由字段带入结果。密钥不提交，真实密钥也不进入文档示例。
...
[和风天气客户端]  [高德地图客户端]  [领域数据合同]
  /v7/weather/3d   地理编码/驾车/POI  Pydantic v2
        │              │               │
        └────── 受控重试、缓存、熔断 ────┘
                       │
                       ▼
